...
const SD=[{numero:1,titulo:'Análisis del modelo clínico',estado:'Completada',nota:'Diagnóstico completo.'},{numero:2,titulo:'Extracción de metodología',estado:'Completada',nota:'Protocolo documentado.'},{numero:3,titulo:'Diseño de la oferta',estado:'Completada',nota:'Oferta completa definida.'},{numero:4,titulo:'Activación del primer grupo',estado:'Proxima',nota:'Selección de pacientes.'},{numero:5,titulo:'Seguimiento del programa',estado:'Pendiente',nota:''},{numero:6,titulo:'Cierre y resultados',estado:'Pendiente',nota:''}]
...
function makeCalendarLink(s,nc){if(!s.fecha_agendada)return null;const f=s.fecha_agendada.replace(/-/g,'');const h=(s.hora||'10:00').replace(':','');const st=f+'T'+h+'00';const eh=String(parseInt(h.slice(0,2))+1).padStart(2,'0');const en=f+'T'+eh+h.slice(2)+'00';const t=encodeURIComponent('Sesión '+s.numero+' — '+s.titulo+' | '+nc);const d=encodeURIComponent((s.link_meet?'Google Meet: '+s.link_meet+'\n\n':'')+'Clínica Escalable™');return 'https://calendar.google.com/calendar/render?action=TEMPLATE&text='+t+'&dates='+st+'/'+en+'&details='+d+(s.link_meet?'&location='+encodeURIComponent(s.link_meet):'')}
...
export default function App(){
  ...
  const navItems=[{id:'dashboard',icon:'◈'},{id:'tareas',icon:'✦'},{id:'sesiones',icon:'◎'},{id:'entregables',icon:'◇'},{id:'carpeta',icon:'📁'},{id:'log',icon:'📋'}]
  ...
  return(<div className='layout'>
    ...
    <main className='main'>
      {!IS_CLIENT_VIEW&&editMode&&(<div className='edit-bar'><label>Fase:</label><select value={c.fase_activa} onChange={e=>cambiarFase(e.target.value)} disabled={saving}>{FASES.map(f=><option key={f} value={f}>{FL[f]}</option>)}</select><div className='flex1'/><button className='btn-green' onClick={()=>setEditMode(false)}>Cerrar edición</button></div>)}

      {/* Modal prompt: completar sesión → coaching log */}
      {showLogPrompt&&(<div className='modal-overlay'><div className='modal'><div className='modal-title'>✅ Sesión completada</div><p style={{fontSize:'14px',color:'var(--text-muted)',marginBottom:'18px',lineHeight:'1.6'}}>¿Quieres registrar las notas de esta sesión en el Coaching Log? Te ayudará a tener un historial completo del proceso.</p><div className='modal-actions'><button className='btn-cancel' onClick={()=>setShowLogPrompt(false)}>Ahora no</button><button className='btn-green' onClick={()=>{setShowLogPrompt(false);setNav('log');setShowLogForm(true)}}>Ir al Coaching Log →</button></div></div></div>)}
      ...
      <div className='topbar'>
        ...
        <div className='topbar-right'>{!IS_CLIENT_VIEW&&(<><button className='btn-edit' onClick={()=>setEditMode(!editMode)}>{editMode?'Cerrar':'✏️ Editar fase'}</button><button className={'btn-copy-link'+(copied?' copied':'')} onClick={copyClientLink}>{copied?'✓ Copiado':'🔗 Copiar link'}</button></>)}</div>
      </div>
      ...
      {proxSesion&&(<div className={'prox-sesion-banner'+(IS_CLIENT_VIEW?' client-banner':'')}>
        <div className='prox-info'>
          ...
          {proxSesion.fecha_agendada?<span className='prox-fecha'>{new Date(proxSesion.fecha_agendada+'T12:00:00').toLocaleDateString('es-MX',{weekday:'long',day:'numeric',month:'long'})}{proxSesion.hora&&' · '+proxSesion.hora}</span>:<span className='prox-sin-fecha'>{IS_CLIENT_VIEW?'Valeria te contactará pronto':'Sin fecha agendada — ve a Sesiones para agendar'}</span>}
        </div>
        <div className='prox-actions'>
          {proxSesion.link_meet&&(<a href={proxSesion.link_meet} target='_blank' rel='noreferrer' className='btn-meet'>📹 Unirse</a>)}
          {!IS_CLIENT_VIEW&&makeCalendarLink(proxSesion,c.nombre)&&(<a href={makeCalendarLink(proxSesion,c.nombre)} target='_blank' rel='noreferrer' className='btn-calendar'>📆 Agendar en Calendar</a>)}
          {!IS_CLIENT_VIEW&&(<button className='btn-agenda-banner' onClick={()=>setNav('sesiones')}>⚙️ Gestionar sesiones</button>)}
        </div>
      </div>)}
      ...
      {nav==='tareas'&&(<div className='card full'>
        ...
        <div className='task-list'>
          {tf.map(t=>(<div key={t.id} className='task-item-edit'>
            ...
            ):(<div className='task-body' style={{flex:1}}><div className={'task-text '+(t.completada?'done':'')}>{t.texto}</div><div className='task-resp-badge'>{t.responsable==='clienta'?'👤 Clienta':'👩‍⚕️ Valeria'}</div></div>)}
        ...
        {!IS_CLIENT_VIEW&&(<div className='nueva-tarea-box'>
          ...
          <div className='nueva-tarea-opciones'>
            <div className='resp-option-group'><span style={{fontSize:'12px',color:'var(--text-muted)',fontWeight:500}}>Responsable:</span><label><input type='radio' name='resp' value='valeria' checked={nuevaResponsable==='valeria'} onChange={()=>setNuevaResponsable('valeria')}/> 👩‍⚕️ Valeria</label><label><input type='radio' name='resp' value='clienta' checked={nuevaResponsable==='clienta'} onChange={()=>setNuevaResponsable('clienta')}/> 👤 Clienta</label></div>
            <div className='resp-option-group'><span style={{fontSize:'12px',color:'var(--text-muted)',fontWeight:500}}>Aplicar a:</span><label><input type='radio' name='aplica' value='solo' checked={aplicarA==='solo'} onChange={()=>setAplicarA('solo')}/> Solo {c.nombre.split(' ')[0]}</label><label><input type='radio' name='aplica' value='todas' checked={aplicarA==='todas'} onChange={()=>setAplicarA('todas')}/> Todas en {fl.split('— ')[1]||'esta fase'}</label></div>
          </div>
      ...
      {nav==='carpeta'&&(<div className='card full'>
        <div className='card-title-row'><span className='card-title'>📁 Carpeta de Drive — {c.nombre.split(' ')[0]}</span></div>
        <div className='carpeta-box'>
          {c.link_drive_carpeta?(<>
            <div className='carpeta-preview'>
              <div className='carpeta-icon'>📁</div>
              ...
              <a href={c.link_drive_carpeta} target='_blank' rel='noreferrer' className='btn-meet'>Abrir carpeta →</a>
            </div>
            {!IS_CLIENT_VIEW&&(<button className='btn-link' style={{marginTop:'12px'}} onClick={()=>setEditDrive(true)}>Cambiar link</button>)}
          </>):(IS_CLIENT_VIEW?(<div className='empty-state'>Tu carpeta de documentos estará disponible pronto.</div>):(<div className='carpeta-empty'>
            <div style={{fontSize:'40px',marginBottom:'12px'}}>📁</div>
            <div style={{fontSize:'15px',fontWeight:500,marginBottom:'6px',color:'var(--text)'}}>Sin carpeta de Drive vinculada</div>
            <div style={{fontSize:'13px',color:'var(--text-muted)',marginBottom:'20px'}}>Vincula la carpeta de Drive donde guardarás todos los documentos de {c.nombre.split(' ')[0]}.</div>
          ...
          {IS_CLIENT_VIEW&&c.link_drive_carpeta&&(<div className='carpeta-msg'>Aquí encontrarás todos los documentos y materiales de tu programa. Puedes acceder en cualquier momento.</div>)}
        </div>
      </div>)}
      ...
      {nav==='log'&&(<div className='card full'>
        ...
        {showLogForm&&(<div className='log-form'>
          ...
          <div className='form-group'><label>Notas de la sesión</label><textarea value={logForm.notas} onChange={e=>setLogForm(p=>({...p,notas:e.target.value}))} placeholder='Qué se trabajó, decisiones, acuerdos...'/></div>
        ...
        <div className='logs-list'>{logs.length===0&&<div className='empty-state'>No hay sesiones registradas aún.</div>}{logs.map(l=>(<div key={l.id} className='log-item'><div className='log-header'><div className='log-title'>{l.sesion_numero?'Sesión '+l.sesion_numero+' — ':''}{l.titulo}</div><div className='log-meta'>{l.fecha&&<span>{l.fecha}</span>}{!IS_CLIENT_VIEW&&<button className='btn-del-log' onClick={()=>deleteLog(l.id)}>×</button>}</div></div>{l.link_fathom&&(<a href={l.link_fathom} target='_blank' rel='noreferrer' className='log-fathom'>▶ Ver grabación en Fathom</a>)}{l.notas&&(<div className='log-notas'>{l.notas}</div>)}{l.tareas_extra&&(<div className='log-tareas'><div className='log-tareas-title'>Tareas extra:</div>{l.tareas_extra.split('\n').filter(Boolean).map((t,i)=>(<div key={i} className='log-tarea-item'>◦ {t}</div>))}</div>)}</div>))}</div>
      </div>)}
    </main>
  </div>)
}
...
function EditTareaInline({texto,responsable,onSave,onCancel}){
  ...
  return(<div className='edit-tarea-inline'>
    <div style={{flex:1}}>
      ...
      <div className='resp-mini'><label><input type='radio' value='valeria' checked={resp==='valeria'} onChange={()=>setResp('valeria')}/> 👩‍⚕️ Valeria</label><label><input type='radio' value='clienta' checked={resp==='clienta'} onChange={()=>setResp('clienta')}/> 👤 Clienta</label></div>
    </div>
...
function SesionItem({s,isAdmin,nombreClienta,onSave,onCompletar}){
  ...
  return(<div className={'session-item big'+(esProxima?' proxima-item':'')}>
    ...
    <div className='session-info' style={{flex:1}}>
      <div className={'session-title '+(esProxima?'next':esPendiente?'faint':'')}>Sesión {s.numero} — {s.titulo}</div>
      <div className={'session-date '+(esProxima?'next':'faint')}>
        {s.estado==='Completada'?'✓ Completada':esProxima?'Próxima':'Pendiente'}
        {s.fecha_agendada&&<strong> · {new Date(s.fecha_agendada+'T12:00:00').toLocaleDateString('es-MX',{weekday:'short',day:'numeric',month:'long'})}</strong>}
        {s.hora&&<strong> {s.hora}</strong>}
      </div>
      {s.nota&&<div className='session-note-text'>{s.nota}</div>}
      {s.link_meet&&(<a href={s.link_meet} target='_blank' rel='noreferrer' className={IS_CLIENT_VIEW?'btn-meet':'log-fathom'} style={{marginTop:'6px',display:'inline-flex'}}>{IS_CLIENT_VIEW?'📹 Unirse a la sesión':'📹 Google Meet'}</a>)}
      {isAdmin&&editing&&(<div className='sesion-edit-box'>
        ...
        <div className='sesion-edit-actions'>
          {calLink&&<a href={calLink} target='_blank' rel='noreferrer' className='btn-calendar'>📆 Crear evento en Google Calendar</a>}
    ...
    {isAdmin&&s.estado!=='Completada'&&(<div className='sesion-actions'>
      <button className={'btn-agenda-sesion'+(esProxima?' proxima':'')} onClick={()=>setEditing(!editing)}>{editing?'Cerrar':'📅 Agendar'}</button>
      {esProxima&&<button className='btn-completar-sesion' onClick={()=>onCompletar(s)}>✓ Completar sesión</button>}
    </div>)}
  </div>)
}
...
function EntregableItem({e,isAdmin,onEstado,onUrl}){
  ...
  return(<div className={'doc-item'+(bloqueado?' locked':'')+' big'}><span className='doc-icon'>{e.icono}</span><div className='doc-body'><span className='doc-name'>{e.nombre}</span>{isAdmin&&editing&&(<div className='url-edit'><input className='url-input' value={urlVal} onChange={ev=>setUrlVal(ev.target.value)} placeholder='Pega el link de Google Drive...'/><button className='btn-save-url' onClick={()=>{onUrl(e.id,urlVal);setEditing(false)}}>Guardar</button><button className='btn-cancel-url' onClick={()=>setEditing(false)}>Cancelar</button></div>)}</div><div className='doc-actions'>{!bloqueado&&e.url&&(<a href={e.url} target='_blank' rel='noreferrer' className='btn-ver'>Ver →</a>)}{isAdmin&&!bloqueado&&(<><button className='btn-link' onClick={()=>setEditing(!editing)}>{e.url?'Editar link':'+ Link'}</button><select className='estado-select' value={e.estado} onChange={ev=>onEstado(e.id,ev.target.value)}>{ESTADOS.map(s=><option key={s} value={s}>{s}</option>)}</select></>)}{(!isAdmin||bloqueado)&&(<span className={'tag '+(e.estado==='Listo'?'tag-green':e.estado==='En curso'?'tag-purple':'tag-gray')}>{bloqueado?'🔒':e.estado}</span>)}</div></div>)
}