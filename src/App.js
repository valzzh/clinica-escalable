...
export default function App(){
  ...
  async function saveUrl(id,url){
    await supabase.from('entregables').update({url}).eq('id',id)
    setEntregables(prev=>prev.map(e=>e.id===id?{...e,url}:e))
    setEditUrl(prev=>({...prev,[id]:false}))
  }
  ...
  // Componente de entregable con link de Drive
  function DocItem({e,isAdmin}){
    const [urlInput,setUrlInput]=useState(e.url||'')
    const bloqueado=e.estado==='Bloqueado'
    return(
      <div className={'doc-item'+(bloqueado?' locked':'')+(isAdmin?' big':'')}>
        <span className='doc-icon'>{e.icono}</span>
        <div className='doc-body'>
          <span className='doc-name'>{e.nombre}</span>
          {isAdmin&&editUrl[e.id]&&(
            <div className='url-edit'>
              <input value={urlInput} onChange={ev=>setUrlInput(ev.target.value)} placeholder='Pega el link de Google Drive aquí...' className='url-input'/>
              <button className='btn-save-url' onClick={()=>saveUrl(e.id,urlInput)}>Guardar</button>
              <button className='btn-cancel-url' onClick={()=>setEditUrl(prev=>({...prev,[e.id]:false}))}>Cancelar</button>
            </div>
          )}
        </div>
        <div className='doc-actions'>
          {!bloqueado&&e.url&&(
            <a href={e.url} target='_blank' rel='noreferrer' className='btn-ver'>Ver documento →</a>
          )}
          {isAdmin&&!editUrl[e.id]&&!bloqueado&&(
            <button className='btn-link' onClick={()=>setEditUrl(prev=>({...prev,[e.id]:true}))}>{e.url?'Editar link':'+ Agregar link'}</button>
          )}
          <span className={'tag '+(e.estado==='Listo'?'tag-green':e.estado==='En curso'?'tag-purple':'tag-gray')}>{bloqueado?'🔒 Bloqueado':e.estado}</span>
        </div>
      </div>
    )
  }
    ...
    <div className='lista-view'>
      ...
      <div className='clientas-grid'>
        ...
        {clientas.map(x=>(<div key={x.id} className='clienta-card' onClick={()=>{setSelId(x.id);setView('dashboard')}}>
          <div className='card-avatar-row'><div className='card-avatar'>{(x.nombre.split(' ').map(p=>p[0]).slice(0,2).join('')).toUpperCase()}</div><div><div className='card-name'>{x.nombre}</div><div className='card-prof'>{x.profesion}</div></div></div>
          ...
          <div className='card-link'>Ver dashboard →</div>
        </div>))}
      </div>
    </div>
  )
  ...
  return(
    <div className='layout'>
      <nav className='sidebar'>
        ...
        {['dashboard','tareas','sesiones','entregables'].map(s=>(<button key={s} className={'nav-item'+(nav===s?' active':'')} onClick={()=>setNav(s)}>{s==='dashboard'?'◈':s==='tareas'?'✦':s==='sesiones'?'◎':'◇'} {s.charAt(0).toUpperCase()+s.slice(1)}</button>))}
      ...
      <main className='main'>
        ...
        <div className='topbar'>
          ...
          {!IS_CLIENT_VIEW&&(<div className='topbar-right'>
            <button className='btn-edit' onClick={()=>setEditMode(!editMode)}>{editMode?'Cerrar':'✏️ Editar'}</button>
            <div className='link-box'><span className='link-label'>Link de clienta:</span><code className='link-code'>?clienta={c.id}</code><button className='btn-copy' onClick={()=>navigator.clipboard.writeText(window.location.origin+'/?clienta='+c.id)}>Copiar</button></div>
          </div>)}
        </div>
        ...
        {nav==='dashboard'&&(<>
          ...
          <div className='cols'>
            ...
            <div className='card'><div className='card-title'>Sesiones del programa</div><div className='session-list'>{sesiones.map(s=>(<div key={s.id} className='session-item'><div className={'session-dot '+(s.estado==='Completada'?'dot-done':s.estado==='Proxima'?'dot-next':'dot-pending')}/><div className='session-info'><div className={'session-title '+(s.estado==='Proxima'?'next':s.estado==='Pendiente'?'faint':'')}>Sesión {s.numero} — {s.titulo}</div><div className={'session-date '+(s.estado==='Proxima'?'next':'faint')}>{s.estado==='Completada'?'Completada':s.estado==='Proxima'?'Por agendar · Próxima':'Pendiente'}</div>{s.nota&&<div className='session-note-text'>{s.nota}</div>}</div></div>))}</div></div>
          </div>
          <div className='cols'>
            <div className='card'><div className='card-title'>Entregables</div><div className='doc-list'>{entregables.map(e=><DocItem key={e.id} e={e} isAdmin={!IS_CLIENT_VIEW}/>)}</div></div>
            <div className='card'>
              {!IS_CLIENT_VIEW&&<><div className='card-title'>Notas internas (solo tú las ves)</div><textarea defaultValue={c.notas_internas||''} placeholder='Notas de sesión...' onBlur={ev=>saveNota(ev.target.value)}/></>}
              {IS_CLIENT_VIEW&&<><div className='card-title'>Tu avance</div><div className='client-message'><p>Estás en <strong>{fl}</strong>.</p><p>Llevas {tf.filter(t=>t.completada).length} de {tf.length} tareas completadas.</p><p>Valeria te contactará pronto para tu próxima sesión.</p></div></>}
            </div>
          </div>
        </>)}
        ...
        {nav==='entregables'&&(<div className='card full'><div className='card-title'>Entregables del programa{!IS_CLIENT_VIEW&&<span className='card-subtitle'> — Haz clic en + Agregar link para vincular documentos de Drive</span>}</div><div className='doc-list'>{entregables.map(e=><DocItem key={e.id} e={e} isAdmin={!IS_CLIENT_VIEW}/>)}</div></div>)}
      </main>
    </div>
  )
}